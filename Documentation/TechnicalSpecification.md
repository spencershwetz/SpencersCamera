...
*   **Recording Service (`RecordingService`)**: Handles video/audio recording using `AVAssetWriter`. Configures inputs/outputs based on selected codec/resolution/log state. Applies orientation transform to video track (using angle derived from `UIDeviceOrientation.videoRotationAngleValue`). Optionally bakes in LUTs using `MetalFrameProcessor`. Saves final file to Photos library.
*   **Settings**: 
