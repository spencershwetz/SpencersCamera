...
*   **Camera Feature**: 
    ...
    *   `ExposureService` sets exposure mode, ISO, shutter, WB, tint based on `CameraViewModel` requests. It also attempts to set `.continuousAutoExposure` upon device initialization (`setDevice`). The final state is synchronized via `CameraSetupService` after the session starts. It uses Key-Value Observing (KVO) to monitor `iso`, `exposureDuration`, and `deviceWhiteBalanceGains` on the `AVCaptureDevice` to provide real-time updates to the delegate even when in automatic modes.
